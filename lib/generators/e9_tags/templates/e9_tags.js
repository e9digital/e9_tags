jQuery(function($) {
  var
  $tag_context = $('#tag-context'), tcv = $.trim($tag_context.val()),
  ...
  escape_context = function(context) {
    return context.replace(/\s+/g, '__S__').replace(/-/, '__D__').replace(/\(hidden\)/, '__H__');
  },

  unescape_context = function(context) {
    return context.replace(/__S__/g, ' ').replace(/__D__/g, '-').replace(/__H__/g, '(hidden)');
  };
  ...
  $(".admin-tag").live("click", function(e) {
    ...
    if (!$li.siblings()) $li.closest('ul').hide();
    $li.remove();
  });
  ...
  $("#add-tag-btn").click(function(evt) {
    ...
    context = $.trim($tcf.val()).replace(/\s+/g, ' ');
